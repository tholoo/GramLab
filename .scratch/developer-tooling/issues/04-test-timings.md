...
The corrected branch integrates without conflicts. All 12 focused CLI checks pass in the
coordinator checkout, with Ruff/format/strict typing. The coordinator reproduces ordinary core
report comparison and uses the command on complete native gates: 29 matched cases, five additions,
no removals, and separate suite versus case duration. Parser-level DTD rejection has a real UTF-16
red/green regression. Shared contributor/CI/environment references now include the command.
