...
source (the exact local path is retained in ignored provenance); only worker-owned
...
Independent review accepts per-complete-response identity publication (a valid pending callback
response may retain disclosed knowledge even if later polling fails). All four source preimages
match, zero-fuzz reconstruction matches exactly and the normal22 APK compiles offline in 2m13s
(11 tasks executed, 67 cached). The previous normal21 APK is a retained negative control: valid
recursive/third-party mention bodies reject; its first host failure is separately identified as
the older probe lacking the new optional history-users observation.

On the immutable normal22 APK, all 75 mention codec cases and all 28 existing photo codec cases
pass in two dedicated serial guest tests. Their full native outputs, isolation assertions, exact
staged source hashes and unchanged APK/input receipt are retained under ignored
`artifacts/rich-mentions-native-01/`. Scoped Ruff/format/mypy pass. This establishes native codecs
and identity envelopes, not original mention UI/live updates: ticket 54 still owns the real-bot
rendering/edit/removal/cold-restart scenario. Keep this ticket claimed until remaining acceptance.
