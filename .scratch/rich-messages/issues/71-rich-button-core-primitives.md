...
Coordinator integration passes 22 focused cases in 0.73 seconds, with scoped Ruff, format and
strict mypy checks across the four source/test files. Byte-cap failures expose
`JournalLimitError`, a `ValueError` subtype, so the registry can distinguish a known capacity
rejection from malformed backend data. Corrupt recovery data remains a validation error.
The coordinator's intermediate failure was an overly broad test exception replacement; its
original red report is retained. World/control integration and actual Android acceptance remain
separate tickets.
