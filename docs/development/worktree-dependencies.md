...
`HEAD`. Receipt evidence must also match its recorded hashes and the actual source/base Git blobs. A modified, staged, committed,
...
batch bytes and exits with an error. If installation rollback also fails, its receipt is retained with state `failed` when the
receipt remains writable, and the CLI reports that recovery is incomplete. Restoration failures
also retain receipt evidence, but repeated filesystem errors may interrupt rollback and leave
its previous state; receipt state alone does not establish that all files were restored. Inspect every named target and all
