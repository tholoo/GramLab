...
def world_with_contacts(
    ...
    *,
    ...
    referenced = world.create_user(first_name=referenced_name, username="mina")
...
def test_user_ids_are_resolved_within_each_world(tmp_path: Path) -> None:
    ...
    second, recipient, bot, local_user, _hidden, chat = world_with_contacts(
        tmp_path / "two", referenced_name="Different local identity"
    )
    with second:
        assert foreign_claim["id"] == local_user["id"]
        assert foreign_claim["first_name"] != local_user["first_name"]
