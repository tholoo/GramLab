"""Feed neutral identity envelopes through the real Android bridge and TL codec."""
...
def probe(guest: Callable[..., subprocess.CompletedProcess[str]]) -> dict[str, object]:
    cases = json.loads(Path("mentions-codec-cases.json").read_text())
    ...
    class FixtureBridge(BaseHTTPRequestHandler):
        ...
        def respond(self, accepted: bool, value: dict[str, Any]) -> None:
        ...
        def do_GET(self) -> None:
            ...
            version = current.get("bridge_version", 3)
        ...
        def do_POST(self) -> None:
            ...
            version = current.get("bridge_version", 3)
            path = "/v3/callbacks" if version == 3 else "/v1/callbacks"
            accepted = (
                self.authorized()
                and self.path == path
    ...
    def adb(*arguments: str, **kwargs: Any) -> None:
        ...
        if result.returncode:
            raise RuntimeError(f"Dedicated mention codec command failed: {arguments[0]}")
    ...
    def observe(case: dict[str, Any], port: int) -> dict[str, Any]:
        ...
        config = {
            ...
            "user_id": 1,
            "bridge_version": case.get("bridge_version", 3),
        }
        adb(
            "shell",
            "-T",
            "sh",
            "-c",
            "'umask 077; cat > /data/local/tmp/gramlab-mentions-codec-config.json'",
            input=json.dumps(config),
        )
        result = guest(
            "shell",
            "CLASSPATH=/data/local/tmp/gramlab-mentions-codec.apk",
            "/system/bin/app_process",
            "/system/bin",
            "org.telegram.gramlab.BridgeProbe",
            "/data/local/tmp/gramlab-mentions-codec-config.json",
            case.get("mode", "mentions"),
            timeout=30,
        )
        if capability in result.stdout or capability in result.stderr:
            raise RuntimeError("Mention codec diagnostics contained a capability")
    ...
    adb("push", "/work/client.apk", "/data/local/tmp/gramlab-mentions-codec.apk", timeout=30)
    try:
        ...
        guest("shell", "rm", "-f", "/data/local/tmp/gramlab-mentions-codec-config.json")
