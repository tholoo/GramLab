"""Observe canonical and malformed message content through the original TL codec.

This isolated fixture server bypasses Python content validation only in the
...
def probe(guest: Callable[..., subprocess.CompletedProcess[str]]) -> dict[str, object]:
    ...
    def adb(*arguments: str, **kwargs: Any) -> None:
        ...
        if result.returncode:
            raise RuntimeError(f"Dedicated snapshot codec command failed: {arguments[0]}")
    ...
    def observe(name: str, content: dict[str, Any] | None) -> dict[str, Any]:
        ...
        result = guest(
            "shell",
            "CLASSPATH=/data/local/tmp/gramlab-codec.apk",
            "/system/bin/app_process",
            "/system/bin",
            "org.telegram.gramlab.BridgeProbe",
            "/data/local/tmp/gramlab-codec-config.json",
            timeout=30,
        )
    ...
    try:
        with HTTPServer(("127.0.0.1", 0), FixtureSnapshot) as server:
            ...
            try:
                adb(
                    ...
                    "sh",
                    "-c",
                    "'umask 077; cat > /data/local/tmp/gramlab-codec-config.json'",
                    input=json.dumps(
                        {
                ...
                cases = json.loads(Path("message-codec-cases.json").read_text())
                return {
                    "baseline": baseline,
                    "cases": {
        ...
        guest("shell", "rm", "-f", "/data/local/tmp/gramlab-codec-config.json")
