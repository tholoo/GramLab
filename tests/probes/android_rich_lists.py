"""Observe trusted canonical list snapshots through the actual Android TL codec.

This isolated fixture server bypasses unfinished Python list validation only in the
...
def probe(guest: Callable[..., subprocess.CompletedProcess[str]]) -> dict[str, object]:
    ...
    def adb(*arguments: str, **kwargs: Any) -> None:
        ...
        if result.returncode:
            raise RuntimeError(f"Dedicated list codec command failed: {arguments[0]}")
    ...
    def observe(name: str, rich: object | None) -> dict[str, Any]:
        ...
        result = guest(
            "shell",
            "CLASSPATH=/data/local/tmp/gramlab-list.apk",
            "/system/bin/app_process",
            "/system/bin",
            "org.telegram.gramlab.BridgeProbe",
            "/data/local/tmp/gramlab-list-config.json",
            timeout=30,
        )
        if token in result.stdout or token in result.stderr:
            raise RuntimeError("List codec diagnostics contained a capability")
        Path(f"{name}-codec.json").write_text(result.stdout)
        return {"returncode": result.returncode, "result": json.loads(result.stdout)}

    adb("push", "/work/client.apk", "/data/local/tmp/gramlab-list.apk", timeout=30)
    try:
        with HTTPServer(("127.0.0.1", 0), FixtureSnapshot) as server:
            ...
            try:
                adb(
                    ...
                    "sh",
                    "-c",
                    "'umask 077; cat > /data/local/tmp/gramlab-list-config.json'",
                    input=json.dumps(
                        {
                ...
                catalog = observe("catalog", json.loads(Path("list-catalog.json").read_text()))
                rejections = {
                    case["name"]: observe(case["name"], case["rich_message"])
                    for case in json.loads(Path("invalid-lists.json").read_text())
                }
                return {"baseline": baseline, "catalog": catalog, "rejections": rejections}
            finally:
                server.shutdown()
                worker.join(timeout=5)
    finally:
        guest("shell", "rm", "-f", "/data/local/tmp/gramlab-list-config.json")
