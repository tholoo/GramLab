"""Manual fixture generation ONLY with the untouched, pinned schema-8 package on PYTHONPATH."""
...
BASE = "df1e67e6c61ba2b63c8883732863aca2e714d6b6"
...
def main() -> None:
    ...
    output.mkdir()
    with tempfile.TemporaryDirectory(prefix="gramlab-document-v8-") as temporary:
        ...
        with closing(sqlite3.connect(directory / "world.sqlite3")) as connection, connection:
            if connection.execute("PRAGMA user_version").fetchone() != (8,):
                raise SystemExit("Expected a genuine schema-8 source database")
