...
javac --release 8 -Xlint:all,-classfile -Werror -classpath "$android_jar:$classes_jar" \
  -d "$output/classes" "$probe_root/DocumentDeliveryProbe.java" "$probe_root/DocumentDeliveryInstrumentation.java" "$probe_root/RenameNoReplaceProbe.java"
...
files = {'source': source / 'DocumentDeliveryProbe.java', 'instrumentation_source': source / 'DocumentDeliveryInstrumentation.java', 'rename_java': source / 'RenameNoReplaceProbe.java', 'compile_script': source / 'compile.sh', 'compiler': javac.resolve(),
